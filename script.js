    ...
    const countdownInterval = setInterval(() => {
        if (countdownValue === 0) {
            ...
            setTimeout(() => {
                ...
                countdownDisplay.innerHTML = "<span class='celebration-text'>Günün neşeyle parlasın ve her anın güzel bir rüya gibi hissettirsin.</span>";
...
sendWishButton.addEventListener('click', () => {
    ...
    wishFeedback.textContent = 'Dilek gönderildi! 🎉';
    burstConfetti();
    setTimeout(() => {
        wishFeedback.textContent = 'Dileğin için teşekkürler!';
    }, 1800);
});
...
function createMemoryIndicators() {
    ...
    memorySlides.forEach((slide, index) => {
        ...
        dot.type = 'button';
        dot.className = 'dot';
        dot.setAttribute('aria-label', `${index + 1}. anıya git`);
